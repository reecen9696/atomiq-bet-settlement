...
PROCESSOR_WALLET=$(solana-keygen pubkey ../keys/test-keypair.json)
USER_WALLET=$(solana-keygen pubkey ../keys/test-user-keypair.json)
...
const processorKeypair = Keypair.fromSecretKey(
  Uint8Array.from(JSON.parse(fs.readFileSync("../keys/test-keypair.json", "utf-8")))
);
const userKeypair = Keypair.fromSecretKey(
  Uint8Array.from(JSON.parse(fs.readFileSync("../keys/test-user-keypair.json", "utf-8")))
);
